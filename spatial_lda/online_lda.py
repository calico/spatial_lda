...
def _update_doc_distribution(X, exp_topic_word_distr, doc_topic_prior,
    ...
    for idx_d in range(n_samples):
        ...
        for _ in range(0, max_iters):
            ...
            if np.isscalar(doc_topic_prior):
                _dirichlet_expectation_1d(doc_topic_d, doc_topic_prior,
                                          exp_doc_topic_d)
            else:
                doc_topic_d += doc_topic_prior[idx_d]
                _dirichlet_expectation_1d(doc_topic_d, 0,
                                          exp_doc_topic_d)
...
class LatentDirichletAllocation(BaseEstimator, TransformerMixin):
    ...
    def set_components(self, components):
        ...
        self._check_params()
        self._init_latent_vars(n_features=n_features)
    ...
    def fit(self, X, y=None):
        ...
        with Parallel(n_jobs=n_jobs, verbose=max(0,
                                                 self.verbose - 1)) as parallel:
            for i in tqdm(range(max_iter), desc='LDA training'):
    ...
    def _approx_bound(self, X, doc_topic_distr, sub_sampling):
        ...
        def _loglikelihood(prior, distr, dirichlet_distr, size):
            ...
            if np.isscalar(prior):
                score += np.sum(gammaln(prior * size) -
                                gammaln(np.sum(distr, 1)))
            else:
                score += np.sum(gammaln(np.sum(prior, 1)) -
                                gammaln(np.sum(distr, 1)))
            return score
